...
public class OpenTsdbProviderTest {
    public OpenTsdbProviderTest() {
        GuiceInjectorHolder.createInjector(Collections.emptyList());
    }

    @Test
    public void getHttp() {
        final OpenTsdbProvider provider = new OpenTsdbProvider(new MetricsOpenTsdbReporterConfiguration() {
            @Override
            public OpenTsdbProtocol getProtocol() {
                return OpenTsdbProtocol.HTTP;
            }
        });
        final OpenTsdb openTsdb = provider.get();
        assertNotNull(openTsdb);
        assertFalse(openTsdb instanceof OpenTsdbTelnet);
    }

    @Test
    public void getTelnet() {
        final OpenTsdbProvider provider = new OpenTsdbProvider(new MetricsOpenTsdbReporterConfiguration() {
            @Override
            public OpenTsdbProtocol getProtocol() {
                return OpenTsdbProtocol.TELNET;
            }
        });
        final OpenTsdb openTsdb = provider.get();
        assertNotNull(openTsdb);
        assertTrue(openTsdb instanceof OpenTsdbTelnet);
    }
}